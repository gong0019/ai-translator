...
每个译文会检查可观测错误，包括结构遗漏、数量变化、术语缺失和不应出现的源语言残留。只有存在缺陷的分块会以温度 `0.0` 定向重译，最多一次。最终译文始终显示，内部校验代码不会输出；若仍有风险，只显示明确说明需要核对内容的提示。该流程会提高严谨性，但长文耗时可能增加约 1.5–2 倍，也不能从数学上证明所有语义绝对等价。
