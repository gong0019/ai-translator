...
新闻等多段文本会逐段处理：每段完成翻译与校验后立刻显示，再继续下一段。单段超过源文本 token 上限时，优先按完整句子拆分，必要时才按安全 token 边界拆分。程序先为全文规划一次共享术语表；完整人名会自动生成姓氏别名，供后续段落保持一致。若当前段大面积重复上一段译文，会只重试当前段。若输入末尾疑似被截断，会先要求确认，不会擅自补写结尾。
...
`full_skill_min_tokens` 以下的短输入只注入通用契约和语向声明，不注入语法规则。规则讲的是长句拆分、段落结构和跨段一致，一句话没有对应的作用对象；而完整提示词约 900 token，翻一句 16 token 的话时全部时间都花在预填上。短句提示词因此从约 900 token 降到约 149。
