...
新闻等多段文本会逐段处理：每段完成翻译与校验后立刻显示，再继续下一段。单段超过源文本 token 上限时，优先按完整句子拆分，必要时才按安全 token 边界拆分。程序先为全文规划一次共享术语表，并把已确认的上一段译文作为仅供衔接的上下文传给下一段，确保人名、指代、时态和重复术语保持一致，同时不延迟前台输出。若输入末尾疑似被截断，会先要求确认，不会擅自补写结尾。
